...
[ApiController]
public class DeleteChave: ControllerBase
{
    [HttpDelete("/chaves")]
    public async Task<IActionResult> HandleAsync([FromQuery] Guid id, [FromServices] ISender sender)
    {
        ...
        var request = new DeleteChaveHandler.Command()
        {
            Id = id
        };
...
public static class DeleteChaveHandler 
{
    ...
    internal sealed class Handler : IRequestHandler<Command, Result<ResultImovel>>
    {
        ...
        public async Task<Result<ResultImovel>> Handle(Command request, CancellationToken cancellationToken)
        {
            try
            {
                var found = await _dbContext.Imoveis.FindAsync(new ChaveId(request.Id), cancellationToken);
                if (found is null)
                {
                    return new Result<ResultImovel>(new ValidationException("Chave não existe"));
                }
